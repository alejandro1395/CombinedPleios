...
metabolic_groups = ["Abnormality of metabolism/homeostasis", "digestive system disease", "metabolic disease"]
immune_groups = ["immune system disease"]
...
def get_subset_MetabolicImmune_Pleiotropies(catalog_fun, pleio_dict):
    ...
    for entry in pleio_dict:
        ...
        subset["PleioType"] = ""
        if any(str(elem) in metabolic_groups for elem in subset['Group'].tolist()) and any(str(elem) in immune_groups for elem in subset['Group'].tolist()):
            immune_subset = subset[subset['Group'].isin(immune_groups)]
            metabolic_subset = subset[subset['Group'].isin(metabolic_groups)]
            immune_riskalleles = retrieve_subset_of_column_values(immune_subset, "Risk_allele")
            metabolic_riskalleles = retrieve_subset_of_column_values(metabolic_subset, "Risk_allele")
            immune_ages = retrieve_subset_of_column_values(immune_subset, "Onset")
            metabolic_ages = retrieve_subset_of_column_values(metabolic_subset, "Onset")
            if set(immune_riskalleles) == set(metabolic_riskalleles):
                type = "Agonistic"
            else:
                type = "Antagonistic"
            pleio_type = define_early_late(age_threeshold, immune_ages, metabolic_ages, type)
...
pleios_metabolicimmune = get_subset_MetabolicImmune_Pleiotropies(catalog, pleiotropic_singles)
pleios_metabolicimmune.to_csv(out_file2, sep='\t')
