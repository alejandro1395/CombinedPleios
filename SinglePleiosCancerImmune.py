...
def get_subset_CancerImmune_Pleiotropies(catalog_fun, pleio_dict):
    ...
    for entry in pleio_dict:
        ...
        subset["PleioType"] = ""
        if any(str(elem) in immune_groups for elem in subset['Group'].tolist()) and any(str(elem) in cancer_groups for elem in subset['Group'].tolist()):
            ...
            if set(cancer_riskalleles) == set(immune_riskalleles):
                type = "Agonistic"
            else:
                type = "Antagonistic"
            pleio_type = define_early_late(age_threeshold, cancer_ages, immune_ages, type)
...
pleios_cancerimmune = get_subset_CancerImmune_Pleiotropies(catalog, pleiotropic_singles)
pleios_cancerimmune.to_csv(out_file2, sep='\t')
