...
cancer_groups = ["neoplasm", "Meningioma"]
...
def get_subset_CancerMetabolic_Pleiotropies(catalog_fun, pleio_dict):
    ...
    for entry in pleio_dict:
        ...
        subset["PleioType"] = ""
        if any(str(elem) in metabolic_groups for elem in subset['Group'].tolist()) and any(str(elem) in cancer_groups for elem in subset['Group'].tolist()):
            cancer_subset = subset[subset['Group'].isin(cancer_groups)]
            metabolic_subset = subset[subset['Group'].isin(metabolic_groups)]
            cancer_riskalleles = retrieve_subset_of_column_values(cancer_subset, "Risk_allele")
            metabolic_riskalleles = retrieve_subset_of_column_values(metabolic_subset, "Risk_allele")
            cancer_ages = retrieve_subset_of_column_values(cancer_subset, "Onset")
            metabolic_ages = retrieve_subset_of_column_values(metabolic_subset, "Onset")
            if set(cancer_riskalleles) == set(metabolic_riskalleles):
                type = "Agonistic"
            else:
                type = "Antagonistic"
            pleio_type = define_early_late(age_threeshold, cancer_ages, metabolic_ages, type)
...
pleios_cancermetabolic = get_subset_CancerMetabolic_Pleiotropies(catalog, pleiotropic_singles)
pleios_cancermetabolic.to_csv(out_file2, sep='\t')
